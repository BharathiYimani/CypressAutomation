...
cy.log('Form submitted and closed successfully');
    ...
    //  no validation message is displayed for valid email format
    //cy.get('div.invalid-feedback > .invalid-feedback').should('not.exist')

    // Type an invalid email address into the input field
    
    //cy.get('input[formcontrolname="email"]').clear().type(invalidEmail)
    //  an invalid validation message is displayed for invalid email format
   // cy.get('span.invalid-feedback').should('exist').then(() => {
      //cy.log('Element with class "invalid-feedback" exists')
    //})
   //cy.get('span.invalid-feedback',{timeout:18000}).should('contain','Invalid')
   //cy.get('span.invalid-feedback').clear().type(validEmail).should('have.value',validEmail)
   // cy.get('input[formcontrolname="email"]').siblings('.invalid-feedback').should('contain','Invalid')
    //cy.get('div.invalid-feedback > .invalid-feedback',{timeout:6000}).should('contain','Invalid')
    //Confirm email validations

    //cy.get('input[placeholder="Confirm Email"]').siblings('.invalid-feedback').should('contain',' Email fields do not match ')
    // Type different email into the confirmation email input field
    //const ConfirmEmail = 'Confirm-email@yopmail.com';
    //cy.get('input[formcontrolname="confirmemail"]').type(ConfirmEmail);
    // Assert that the validation message is displayed
    //cy.get(':nth-child(8) > div.ng-star-inserted > .invalid-feedback',{timeout:6000}).should('contain','Email fields do not match');
    //cy.get('input[placeholder="Confirm Email"]').clear().type(validEmail)
